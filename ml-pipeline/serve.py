...
async def score_bytecode(request: ScoringRequest) -> ScoringResponse:
    ...
    try:
        ...
        features = extract_bytecode_features(request.bytecode)

        # Run inference
        prediction = model.predict(features)

        threat_score = prediction["threat_score"]
        vuln_type = prediction["vuln_type"]
        confidence = prediction["confidence"]

        elapsed_ms = (time.perf_counter() - start) * 1000

        logger.info(
            f"GNN inference for tx {request.tx_hash}: "
            f"score={threat_score:.3f}, type={vuln_type}, conf={confidence:.3f}"
        )
