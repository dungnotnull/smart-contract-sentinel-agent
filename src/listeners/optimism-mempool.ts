/**
 * OptimismMempoolListener — Optimism mempool subscription for pending transactions.
 * Optimism uses a traditional mempool similar to Ethereum.
...
export class OptimismMempoolListener extends BaseChainListener {
  ...
  private async connect(): Promise<void> {
    try {
      ...
      this.unwatch = this.client.watchPendingTransactions({
        ...
        onError: (error: Error) => {
          logger.error({ chain: this.config.chainName, err: error }, "Optimism mempool subscription error");
          void this.reconnect(() => this.connect());
        },
      });

      this.isConnected = true;
      logger.info({ chain: this.config.chainName, wsUrl }, "Optimism listener subscribed");
    } catch (error) {
      logger.error({ chain: this.config.chainName, err: error }, "Failed to connect Optimism listener");
      void this.reconnect(() => this.connect());
    }
  }
  ...
  async unsubscribe(): Promise<void> {
    ...
    logger.info({ chain: this.config.chainName }, "Optimism listener unsubscribed");
  }
