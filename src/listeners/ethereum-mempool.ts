/**
 * EthereumMempoolListener â€” WebSocket subscription to pending transactions.
 * Uses viem createPublicClient with WebSocket transport for real-time mempool.
...
export class EthereumMempoolListener extends BaseChainListener {
  ...
  // callback is inherited from BaseChainListener
  ...
  async subscribe(callback: TransactionCallback): Promise<void> {
    // callback is set via subscribe() which stores it in base class
  ...
  private async connect(): Promise<void> {
    try {
      ...
      this.unwatch = this.client.watchPendingTransactions({
        ...
        onError: (error: Error) => {
          logger.error({ chain: this.config.chainName, err: error }, "Mempool subscription error");
          void this.reconnect(() => this.connect());
        },
      });

      this.isConnected = true;
      logger.info({ chain: this.config.chainName, wsUrl }, "Mempool listener subscribed");
    } catch (error) {
      logger.error({ chain: this.config.chainName, err: error }, "Failed to connect mempool listener");
      void this.reconnect(() => this.connect());
    }
  }
  ...
  async unsubscribe(): Promise<void> {
    ...
    logger.info({ chain: this.config.chainName }, "Mempool listener unsubscribed");
  }

  getStatus(): { connected: boolean; chainName: string; chainId: number; txCount: number; rpcPool: ReturnType<RpcPool["getStatus"]> } {
    return {
