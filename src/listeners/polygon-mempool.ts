/**
 * PolygonMempoolListener — Polygon mempool subscription for pending transactions.
 * Polygon uses a traditional mempool with MATIC/POL as native token.
...
export class PolygonMempoolListener extends BaseChainListener {
  ...
  private async connect(): Promise<void> {
    try {
      ...
      this.unwatch = this.client.watchPendingTransactions({
        ...
        onError: (error: Error) => {
          logger.error({ chain: this.config.chainName, err: error }, "Polygon mempool subscription error");
          void this.reconnect(() => this.connect());
        },
      });

      this.isConnected = true;
      logger.info({ chain: this.config.chainName, wsUrl }, "Polygon listener subscribed");
    } catch (error) {
      logger.error({ chain: this.config.chainName, err: error }, "Failed to connect Polygon listener");
      void this.reconnect(() => this.connect());
    }
  }
  ...
  async unsubscribe(): Promise<void> {
    ...
    logger.info({ chain: this.config.chainName }, "Polygon listener unsubscribed");
  }
